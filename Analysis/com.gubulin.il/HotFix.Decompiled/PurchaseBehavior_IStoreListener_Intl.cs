...
public class PurchaseBehavior_IStoreListener_Intl
{
	...
	public PurchaseBehavior_IStoreListener_Intl()
	{
		_purchaseInitTaskCompletionSource = new CustomTaskCompletionSource<bool>();
	}
	...
	private void OnProductsFetchFailed(ProductFetchFailed failureDesc)
	{
		...
		string text = "PurchaseBehavior_Intl UnityPurchasing OnProductsFetchFailed:" + failureDesc.FailureReason;
	...
	private void ProcessPurchase()
	{
		...
		if (LastPendingOrder != null)
		{
			...
			string receipt = info.Receipt;
			if ((int)Application.platform == 8)
			{
			}
			if (lastPendingOrderId > 0)
			{
				CheckOrderAction?.Invoke(lastPendingOrderId.ToString(), productId, transactionID, receipt, 5);
				return;
			}
			Task<SyncPendingReceiptsResponse> task = GameController.Contexts.Service<INetworkService>().SyncPendingReceipts(productId, receipt);
