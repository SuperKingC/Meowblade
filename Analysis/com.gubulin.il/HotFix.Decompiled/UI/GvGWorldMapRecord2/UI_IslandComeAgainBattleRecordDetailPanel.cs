...
public class UI_IslandComeAgainBattleRecordDetailPanel : GComponent, IUiController
{
	...
	public const string URL = "ui://5xc1njmujjrn21";

	public static string Name = "UI_IslandComeAgainBattleRecordDetailPanel";

	private GvGMode2BattleReportBattleRecord UserInfo;
	...
	{
		return "ui://5xc1njmujjrn21";
	}

	public static UI_IslandComeAgainBattleRecordDetailPanel CreateInstance()
	{
		return (UI_IslandComeAgainBattleRecordDetailPanel)(object)UIPackage.CreateObject("GvGWorldMapRecord2", "IslandComeAgainBattleRecordDetailPanel");
	}

	public static UI_IslandComeAgainBattleRecordDetailPanel CreateInstance_ILRuntime()
	{
		//IL_0028: Unknown result type (might be due to invalid IL or missing references)
		//IL_0032: Expected O, but got Unknown
		string full_name = typeof(UI_IslandComeAgainBattleRecordDetailPanel).FullName;
		UIObjectFactory.SetPackageItemExtension("ui://5xc1njmujjrn21", (GComponentCreator)(() => HotFixManager.Instance.appdomain.Instantiate<GComponent>(full_name, (object[])null)));
	...
	public override void ConstructFromXML(XML xml)
	{
		...
		string id = "ui://5xc1njmujjrn21".Replace("ui://", "") + "-" + ((GObject)n11).id;
		...
		string id2 = "ui://5xc1njmujjrn21".Replace("ui://", "") + "-" + ((GObject)n21).id;
	...
	public void Init(Dictionary<string, object> parameters)
	{
		...
		{
			UserInfo = (GvGMode2BattleReportBattleRecord)value;
		}
		...
		if (parameters.TryGetValue("IsInZone", out var value5))
		{
			IsInZone = (bool)value5;
		}
		...
		{
			ILRuntimeDebug.LogError("Open UI_IslandComeAgainBattleRecordDetailPanel GvGRecordDetailData is null");
		}
	}
	...
	private void PlayReplay()
	{
		...
		Singleton<GvGInstanceZone>.Instance.RecordLevelInfo = new GvGInstanceZone.GvGMode2BattleInfo
		{
			BattleId = BattleResultData.BattleId,
			LevelId = GvGReplayLevelId,
		...
		Singleton<GvGInstanceZone>.Instance.OnReplayStart();
		if (IsInZone)
		{
			GvGWorldMapController.Instance.PauseInstance();
		}
	...
	private void OpenDamageMeter()
	{
		Dictionary<string, object> parameters = new Dictionary<string, object>
		{
			...
			{ "IsIslandComeAgain", true },
			{ "IslandComeAgainBlueSoldierDetails", BlueDetail.Soldiers },
			{ "IslandComeAgainRedDetails", RedDetail.Soldiers }
		};
		...
		for (int i = 0; i < enemyFormations.Count; i++)
		{
			if (i <= _curSoldiers.Count - 1)
			{
				...
				if (!string.IsNullOrWhiteSpace(soldierId) && soldierId != "Unlock" && soldierId != "Lock")
				{
					...
					((GComponent)enemyFormations[i]).GetChild("num").text = $"{num2}/{soldierFormationNumber}";
				}
				else
				{
					((GComponent)enemyFormations[i]).GetController("Type").selectedIndex = 0;
