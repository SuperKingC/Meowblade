...
public class UI_PeakBattleSketchMap : GComponent
{
	...
	public const string URL = "ui://82mo10n5x1jlddl";
	...
	{
		return "ui://82mo10n5x1jlddl";
	}

	public static UI_PeakBattleSketchMap CreateInstance()
	{
		return (UI_PeakBattleSketchMap)(object)UIPackage.CreateObject("PvpSelectSoldiers", "PeakBattleSketchMap");
	}
	...
	public static UI_PeakBattleSketchMap CreateInstance_ILRuntime()
	{
		...
		UIObjectFactory.SetPackageItemExtension("ui://82mo10n5x1jlddl", (GComponentCreator)(() => HotFixManager.Instance.appdomain.Instantiate<GComponent>(full_name, (object[])null)));
	...
	public void OnCampClose(EventContext eventContext, string soldierId, int chosenType)
	{
		...
		if (string.IsNullOrEmpty(soldierId) || soldierId == "Lock" || soldierId == "Unlock")
		{
			...
			UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel?.UpdateSomeSoldierBtn(_curSelectedIndex, soldierId);
			UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel?.UpdateSelectedSoldierId(sid, isAdd: false);
		}
		else if (!_curSoldiers.Contains(soldierId))
		{
			...
			_curSoldiers[_curSelectedIndex] = soldierId;
			UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel?.UpdateSomeSoldierBtn(_curSelectedIndex, soldierId);
			UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel?.UpdateSelectedSoldierId(sid2, isAdd: false);
			UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel?.UpdateSelectedSoldierId(soldierId, isAdd: true);
		}
		else
		{
			...
			UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel?.UpdateSomeSoldierBtn(_curSelectedIndex, soldierId);
			UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel?.UpdateSomeSoldierBtn(index, text);
		}
		_curSelectedSoldiers = UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel?.selectedSoldierId;
		if (!_legendItemIds.ContainsKey(soldierId))
		{
			List<long> value = new List<long> { 0L, 0L };
			if (UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel != null)
			{
				SoldierWithLegendItemId soldierInfo = UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel.GetSoldierInfo(soldierId);
	...
	public void OnBlockTouchEnd(EventContext context)
	{
		...
		if (touchTarget != null && touchTarget.gameObjectName == "soliderItem" && _curSelectedIndex != -1)
		{
			...
			if (num <= _curSoldiers.Count - 1)
			{
				...
				UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel?.UpdateSomeSoldierBtn(_curSelectedIndex, text);
				UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel?.UpdateSomeSoldierBtn(num, text2);
				GetMySoldiersCombatPower();
				SetOurFormations(_curSoldiers);
				ShowOurIcons();
			}
	...
	private void GetMySoldiersCombatPower()
	{
		...
		((GObject)UI_PeakBattleSelectArrayPanel.PeakBattleSelectArrayPanel.Dialog.OurCombat).text = num.ToString();
	}
