...
public class UI_MyTroopsSketchMap : GComponent
{
	...
	public const string URL = "ui://4eq8fgd2aibkb6sdt";
	...
	{
		return "ui://4eq8fgd2aibkb6sdt";
	}

	public static UI_MyTroopsSketchMap CreateInstance()
	{
		return (UI_MyTroopsSketchMap)(object)UIPackage.CreateObject("GvGWorldMap3", "MyTroopsSketchMap");
	}
	...
	public static UI_MyTroopsSketchMap CreateInstance_ILRuntime()
	{
		...
		UIObjectFactory.SetPackageItemExtension("ui://4eq8fgd2aibkb6sdt", (GComponentCreator)(() => HotFixManager.Instance.appdomain.Instantiate<GComponent>(full_name, (object[])null)));
	...
	public void PreLoadFormations()
	{
		FormationsInit();
	}

	public void SetOurPos(string fid, List<GvGMode3UnitInfo> currentUnitInfo)
	{
	...
	private void RenderLegendItems(GvGMode3UnitInfo unitInfo, GButton button)
	{
		...
		for (int j = 0; j < array.Length && j < 2; j++)
		{
			...
			long num2 = array[j];
			if (!soldierItemSlotState || num2 <= 0)
			{
				asLoader.url = "";
