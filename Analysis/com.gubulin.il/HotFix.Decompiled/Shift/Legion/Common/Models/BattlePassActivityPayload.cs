...
public class BattlePassActivityPayload : ActivityContentPayload
{
	...
	public BattlePassActivityPayload(int contentPayloadIndex, Dictionary<string, object> data, Activity activity)
		: base(data)
	{
		...
		if (data.TryGetValue("ScoreItem", out var value4))
		{
			...
			if (GDMgr.Get<GDEItemData>(ScoreItem) == null)
			{
				throw new Exception("BattlePass 的 ScoreItem 不是物品");
			}
		}
		if (data.TryGetValue("PaidCert", out var value5))
		{
			PaidCert = value5.ToString();
			if (!string.IsNullOrEmpty(PaidCert) && GDMgr.Get<GDEItemData>(PaidCert) == null)
			{
