...
public class GvGTipsManager
{
	...
	private static GvGTipsManager _Instance;
	...
	public static GvGTipsManager Instance
	{
		get
		{
			if (_Instance == null)
			{
				_Instance = new GvGTipsManager();
			}
			return _Instance;
		}
	}

	private GvGTipsManager()
	{
	...
	private IEnumerator PlayTipsCotoutine()
	{
		while (TipsParams.Count > 0)
		{
			if (Time.time >= TipsParams[0].ShowTimeStamp)
			{
				...
				GameController.Contexts.Service<IUiService>().OpenPanel(UI_GvGTip.Name, tipData.UiParam, multiMode: true);
			}
			yield return null;
		}
