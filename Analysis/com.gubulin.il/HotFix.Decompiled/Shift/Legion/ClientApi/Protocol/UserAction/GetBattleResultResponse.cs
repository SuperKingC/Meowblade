...
[ProtoContract]
public class GetBattleResultResponse : IPacketBody
{
	[ProtoMember(1)]
	public bool Result;
	...
	[ProtoMember(30)]
	public bool CanBackInTime;

	[ProtoMember(31)]
	public int ContractFreeBackInTimeTimes;

	[ProtoMember(32)]
	public int PaidBackInTimeCount;

	[ProtoMember(33)]
	public string _pbBackInTimeCost;

	private Dictionary<string, int> _backInTimeCost;
	...
	public List<List<float>> RedTeamHp
	{
		get
		{
			...
			List<List<float>> list = _redTeamHp ?? (_redTeamHp = JsonHelper.ToObject<List<List<float>>>(_pbRedTeamHp));
			if (list == null)
			{
				return new List<List<float>>();
			}
	...
	public List<List<float>> BlueTeamHp
	{
		get
		{
			...
			List<List<float>> list = _blueTeamHp ?? (_blueTeamHp = JsonHelper.ToObject<List<List<float>>>(_pbBlueTeamHp));
			if (list == null)
			{
				return new List<List<float>>();
			}
	...
	public Dictionary<string, int> BackInTimeCost
	{
		get
		{
			if (_pbBackInTimeCost == null)
			{
				return null;
			}
			return _backInTimeCost ?? (_backInTimeCost = JsonHelper.ToObject<Dictionary<string, int>>(_pbBackInTimeCost));
		}
		set
		{
			_backInTimeCost = value;
			_pbBackInTimeCost = JsonHelper.ToJson(value);
		}
	}
