...
internal sealed class TypeSerializer : IProtoTypeSerializer, IProtoSerializer
{
	...
	private IProtoSerializer GetMoreSpecificSerializer(object value)
	{
		...
		Type pType = PType.GetPType(value);
		if (pType == forType)
		{
			return null;
		}
		for (int i = 0; i < serializers.Length; i++)
		{
			...
			if (protoSerializer.ExpectedType != forType && Helpers.IsAssignableFrom(protoSerializer.ExpectedType, pType))
			{
				return protoSerializer;
			}
		}
		if (pType == constructType)
		{
			return null;
		}
		TypeModel.ThrowUnexpectedSubtype(forType, pType);
		return null;
	}
