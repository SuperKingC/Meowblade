...
public class MetaType : ISerializerProxy
{
	...
	public Type KVPKeyType { get; set; }

	public Type KVPValueType { get; set; }
	...
	private IProtoTypeSerializer BuildSerializer()
	{
		...
		if (IsAutoTuple)
		{
			...
			return new TupleSerializer(model, constructorInfo, mappedMembers, new Type[2] { KVPKeyType, KVPValueType });
		}
		fields.Trim();
	...
	internal void ApplyDefaultBehaviourImpl()
	{
		...
		if (type != null && model.FindWithoutAdd(type) == null && GetContractFamily(model, type, null) != AttributeFamily.None)
		{
		...
		if (flag3 || implicitFields != ImplicitFields.None)
		{
			Array.Sort(array6);
			int num2 = num;
			ProtoMemberAttribute[] array7 = array6;
		...
		ProtoMemberAttribute[] array8 = array6;
		foreach (ProtoMemberAttribute normalizedAttribute in array8)
		{
	...
	internal static AttributeFamily GetContractFamily(RuntimeTypeModel model, Type type, AttributeMap[] attributes)
	{
		...
		if (attributeFamily == AttributeFamily.None && ResolveTupleConstructor(type, out var _) != null)
		{
	...
	internal static ConstructorInfo ResolveTupleConstructor(Type type, out MemberInfo[] mappedMembers)
	{
		...
		return (num == 1) ? constructorInfo : null;
	}
	...
	private static ProtoMemberAttribute NormalizeProtoMember(TypeModel model, MemberInfo member, AttributeFamily family, bool forced, bool isEnum, BasicList partialMembers, int dataMemberOffset, bool inferByTagName, ref bool hasConflictingEnumValue, MemberInfo backingMember = null)
	{
		...
		if (!ignore && !flag)
		{
			...
			if (!ignore && attribute != null)
			{
				...
				GetFieldTypeName(ref typeName, attribute, "TypeName");
				GetFieldTypeName(ref typeName2, attribute, "KeyTypeName");
	...
	private ValueMember ApplyDefaultBehaviour(bool isEnum, ProtoMemberAttribute normalizedAttribute)
	{
		...
		if (itemType != null)
		{
			...
			if (num >= 0 && (flag = model[memberType].IgnoreListHandling))
			{
				itemType = null;
				defaultType = null;
			}
		...
		string typeName = normalizedAttribute.TypeName;
		...
		if (valueMember != null)
		{
			...
			valueMember.IsMap = !flag && valueMember.ResolveMapTypes(out var _, out var _, out var _);
			if (valueMember.IsMap && (attribute = GetAttribute(attribs, "ProtoBuf.ProtoMapAttribute")) != null)
			{
				...
				else
				{
					...
					valueMember.MapKeyTypeName = normalizedAttribute.KeyTypeName;
					valueMember.MapValueTypeName = normalizedAttribute.TypeName;
				}
			}
		}
		return valueMember;
	...
	{
		if (attrib != null && string.IsNullOrEmpty(typeName) && attrib.TryGet(memberName, out var value) && value != null)
		{
	...
	private ValueMember AddField(int fieldNumber, string memberName, Type itemType, Type defaultType, object defaultValue)
	{
		...
		ValueMember valueMember = new ValueMember(model, type, fieldNumber, memberInfo2 ?? memberInfo, memberType, itemType, defaultType, DataFormat.Default, defaultValue);
		if (memberInfo2 != null)
		{
	...
	internal void WriteSchema(StringBuilder builder, int indent, ref RuntimeTypeModel.CommonImports imports, ProtoSyntax syntax)
	{
		...
		if (IsAutoTuple)
		{
			if (!(ResolveTupleConstructor(type, out var mappedMembers) != null))
			{
				return;
			}
			NewLine(builder, indent).Append("message ").Append(GetSchemaTypeName()).Append(" {");
		...
		if (Helpers.IsEnum(type))
		{
			...
			else
			{
				...
				foreach (ValueMember valueMember3 in array5)
				{
		...
		foreach (ValueMember valueMember4 in array6)
		{
			bool hasOption = false;
			string schemaTypeName3;
			if (valueMember4.IsMap)
			{
				valueMember4.ResolveMapTypes(out var _, out var keyType, out var valueType);
			...
			else
			{
				...
				if (syntax != ProtoSyntax.Proto2 && valueMember4.DefaultValue != null && !valueMember4.IsRequired && !IsImplicitDefault(valueMember4.DefaultValue))
				{
		...
		if (subTypes != null && subTypes.Count != 0)
		{
			SubType[] array7 = new SubType[subTypes.Count];
			subTypes.CopyTo(array7, 0);
			Array.Sort(array7, SubType.Comparer.Default);
			string[] array8 = new string[array7.Length];
			for (int num = 0; num < array7.Length; num++)
			{
				array8[num] = array7[num].DerivedType.GetSchemaTypeName();
			}
			string text = "subtype";
			while (Array.IndexOf(array8, text) >= 0)
			{
				text = "_" + text;
			}
	...
	public void ApplyFieldOffset(int offset)
	{
		...
		try
		{
			...
			if (fields != null)
			{
				...
				{
					ValueMember valueMember = (ValueMember)enumerator.Current;
					AssertValidFieldNumber(valueMember.FieldNumber + offset);
				}
			}
			if (subTypes != null)
			{
			...
			if (fields != null)
			{
				BasicList.NodeEnumerator enumerator3 = fields.GetEnumerator();
				while (enumerator3.MoveNext())
				{
					((ValueMember)enumerator3.Current).FieldNumber += offset;
				}
			}
			if (subTypes != null)
			{
				BasicList.NodeEnumerator enumerator4 = subTypes.GetEnumerator();
				while (enumerator4.MoveNext())
				{
					((SubType)enumerator4.Current).FieldNumber += offset;
				}
			}
		}
		finally
