...
public class CampType6Parser : IMessageParser<eChatSystemTemplateType, ChatSystemMessageData>
{
	public HashSet<eChatSystemTemplateType> CanParse()
	{
		return new HashSet<eChatSystemTemplateType>
		{
			eChatSystemTemplateType.CampType6,
			eChatSystemTemplateType.CampType7
		};
	}
	...
	public ChatSystemMessageData Parse(List<object> messageList, eChatThemeType textType)
	{
		...
		string text2 = GvG3ProfileHelper.TryGetUserProfile(num)?.Name ?? $"{num}";
		string curIZIslandName = WorldMapConfigHelper.GetCurIZIslandName(num2);
		string messageText = string.Format(langKey.ToLanguage(), new object[3] { text2, num2, curIZIslandName });
