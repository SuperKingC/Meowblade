...
public class Script_EnterThemeDungeonPanel : ThemeDungeonClickSimulatorStep
{
	public Script_EnterThemeDungeonPanel(ThemeDungeonLevelLocator levelLocator)
	{
	...
	public override IEnumerator Execute()
	{
		...
		for (int i = 0; i < panel.CardLoader.cardList.numItems; i++)
		{
			...
			if (((GObject)entrance).touchable && entrance.TypeController.selectedIndex == 0)
			{
				((GButton)entrance).FireClick(true, false);
				UI_MaskCover.OnTouchBegin((GButton)(object)entrance);
				((GObject)entrance).onClick.Call();
				NextStep = new Script_ThemeDungeonChooseLevel(LevelLocator);
