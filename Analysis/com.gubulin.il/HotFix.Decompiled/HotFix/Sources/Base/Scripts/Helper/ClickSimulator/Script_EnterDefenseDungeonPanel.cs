...
public class Script_EnterDefenseDungeonPanel : DefenseDungeonClickSimulatorStep
{
	public Script_EnterDefenseDungeonPanel(DefenseDungeonLevelLocator levelLocator)
	{
	...
	public override IEnumerator Execute()
	{
		...
		for (int i = 0; i < panel.CardLoader.cardList.numItems; i++)
		{
			...
			if (((GObject)entrance).touchable && entrance.TypeController.selectedIndex == 2)
			{
				((GButton)entrance).FireClick(true, false);
				UI_MaskCover.OnTouchBegin((GButton)(object)entrance);
				((GObject)entrance).onClick.Call();
				NextStep = new Script_DefenseDungeonChooseLevel(LevelLocator);
